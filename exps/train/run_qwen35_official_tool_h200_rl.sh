...
export PROMPT_ADMISSION_POOL_SIZE=${PROMPT_ADMISSION_POOL_SIZE:-128}
...
if (( TRAIN_BATCH_SIZE % AGENT_NUM_WORKERS != 0 )); then
  echo "TRAIN_BATCH_SIZE must be divisible by AGENT_NUM_WORKERS for agent-loop rollout chunking: ${TRAIN_BATCH_SIZE} vs ${AGENT_NUM_WORKERS}" >&2
  exit 2
fi
