...
USER_FORWARD_VARS=(
  AGENTS_FILE DEEPSEEK_API_KEY HF_TOKEN HUGGING_FACE_HUB_TOKEN \
  PYTHON_CMD HF_HOME WANDB_MODE TOKENIZERS_PARALLELISM HYDRA_FULL_ERROR VLLM_USE_V1 \
  OMP_NUM_THREADS MKL_NUM_THREADS OPENBLAS_NUM_THREADS NUMEXPR_NUM_THREADS \
  MINIO3_STAGE_LOG MINIO3_TRAJ_STATUS_INTERVAL_S \
  DATA_DIR TRAIN_FILE VAL_FILE \
  VERL_FILE_LOGGER_PATH ROLLOUT_DATA_DIR VALIDATION_DATA_DIR TRAIN_SAMPLES_JSONL \
  TRAIN_SAMPLES_JSONL_LIMIT \
  TRAIN_BATCH_SIZE VAL_BATCH_SIZE PPO_MINI_BATCH_SIZE \
  PPO_MICRO_BATCH_SIZE_PER_GPU LOG_PROB_MICRO_BATCH_SIZE_PER_GPU \
  REF_LOG_PROB_MICRO_BATCH_SIZE_PER_GPU PPO_MAX_TOKEN_LEN_PER_GPU \
  MAX_PROMPT_LENGTH MAX_RESPONSE_LENGTH VAL_RESPONSE_LENGTH MAX_MODEL_LEN \
  MAX_NUM_BATCHED_TOKENS MAX_NUM_SEQS ROLLOUT_N AGENT_NUM_WORKERS MAX_NUM_GEN_BATCHES \
  CUDA_VISIBLE_DEVICES NGPUS_PER_NODE \
  MINIO3_TOOL_PROMPT_SUITE MINIO3_OFFICIAL_TOOL_NAME MINIO3_AGENT_LOOP ROLLOUT_AGENT_LOOP \
  ROLLOUT_MULTI_TURN_FORMAT ADD_VISION_ID MINIO3_SHOW_BUDGET \
  ROLLOUT_DISABLE_MM_PREPROCESSOR_CACHE ROLLOUT_SKIP_VLLM_DUMMY_LORA \
  ROLLOUT_VLLM_EXECUTOR_BACKEND ROLLOUT_ENFORCE_EAGER ROLLOUT_FREE_CACHE_ENGINE \
  ...
  VAL_BEFORE_TRAIN VAL_ONLY TOTAL_EPOCHS SELF_JUDGE_REWARD SELF_JUDGE_PROVIDER \
  SELF_JUDGE_MODEL SELF_JUDGE_BASE_URL SELF_JUDGE_REASONING_EFFORT SELF_JUDGE_MAX_TOKENS \
  SELF_JUDGE_TEMPERATURE SELF_JUDGE_MAX_RETRIES LLM_JUDGE_MAX_RETRIES SELF_JUDGE_TIMEOUT \
  SELF_JUDGE_INITIAL_DELAY SELF_JUDGE_RELAXED_ANSWER \
  MINIO3_IGNORE_CLIP MINIO3_IGNORE_EXCEED MINIO3_IGNORE_FORMAT MINIO3_IGNORE_INVALID \
  PROMPT_ADMISSION_ENABLE PROMPT_ADMISSION_POOL_SIZE PROMPT_ADMISSION_REWARD_STD_EPSILON \
  PROMPT_ADMISSION_WAIT_TIMEOUT_S PROMPT_ADMISSION_CANCEL_UNFINISHED \
  PROMPT_ADMISSION_ABORT_UNFINISHED_REQUESTS PROMPT_ADMISSION_STATE_PATH \
  LOGGER_BACKENDS TRAIN_MAX_SAMPLES VAL_MAX_SAMPLES \
  ...
  CLIP_RATIO_HIGH CLIP_RATIO_LOW USE_DYNAMIC_BSZ ACTOR_PARAM_OFFLOAD ACTOR_OPTIMIZER_OFFLOAD \
...
export DATA_DIR=${DATA_DIR:-$PROJECT_DIR/data/minio3_real_train_h200}
...
export MINIO3_SHOW_BUDGET=${MINIO3_SHOW_BUDGET:-False}
...
if [[ "${MINIO3_PRINT_CONFIG_ONLY:-0}" == "1" ]]; then
  for name in \
    MODE RUN_ID RUN_DIR MODEL_PATH PYTHON_CMD HF_HOME MINIO3_TOOL_PROMPT_SUITE MINIO3_OFFICIAL_TOOL_NAME \
    MINIO3_AGENT_LOOP ROLLOUT_MULTI_TURN_FORMAT TRAIN_BATCH_SIZE PPO_MINI_BATCH_SIZE \
    PPO_MICRO_BATCH_SIZE_PER_GPU PPO_MAX_TOKEN_LEN_PER_GPU USE_DYNAMIC_BSZ \
    DATA_DIR TRAIN_FILE VAL_FILE TRAIN_MAX_SAMPLES VAL_MAX_SAMPLES FILTER_OVERLONG_PROMPTS_WORKERS \
    ...
    ROLLOUT_GPU_MEM_UTIL ROLLOUT_VLLM_EXECUTOR_BACKEND ROLLOUT_DISABLE_MM_PREPROCESSOR_CACHE \
    ROLLOUT_SKIP_VLLM_DUMMY_LORA \
    ADD_VISION_ID MINIO3_SHOW_BUDGET \
    MAX_ASSISTANT_TURNS MAX_USER_TURNS VAL_MAX_ASSISTANT_TURNS VAL_MAX_USER_TURNS \
    AGENT_NUM_WORKERS RAY_NUM_CPUS ACTOR_PARAM_OFFLOAD ACTOR_OPTIMIZER_OFFLOAD REF_PARAM_OFFLOAD \
    SELF_JUDGE_REWARD SELF_JUDGE_PROVIDER SELF_JUDGE_MODEL SELF_JUDGE_BASE_URL \
    SELF_JUDGE_REASONING_EFFORT SELF_JUDGE_MAX_TOKENS SELF_JUDGE_TEMPERATURE \
    SELF_JUDGE_MAX_RETRIES LLM_JUDGE_MAX_RETRIES SELF_JUDGE_TIMEOUT SELF_JUDGE_INITIAL_DELAY \
    SELF_JUDGE_RELAXED_ANSWER PROMPT_ADMISSION_ENABLE \
    MINIO3_IGNORE_CLIP MINIO3_IGNORE_EXCEED MINIO3_IGNORE_FORMAT MINIO3_IGNORE_INVALID \
    PROMPT_ADMISSION_POOL_SIZE PROMPT_ADMISSION_REWARD_STD_EPSILON \
    PROMPT_ADMISSION_WAIT_TIMEOUT_S PROMPT_ADMISSION_CANCEL_UNFINISHED \
    PROMPT_ADMISSION_ABORT_UNFINISHED_REQUESTS PROMPT_ADMISSION_STATE_PATH \
    ACTOR_LR LORA_RANK LORA_ALPHA LORA_TARGET_MODULES \
    TOTAL_EPOCHS TOTAL_TRAINING_STEPS SAVE_FREQ TEST_FREQ LOGGER_BACKENDS WANDB_MODE \
    TOKENIZERS_PARALLELISM HYDRA_FULL_ERROR VLLM_USE_V1 MINIO3_STAGE_LOG MINIO3_TRAJ_STATUS_INTERVAL_S \
    GPU_MONITOR_ENABLE GPU_MONITOR_INTERVAL GPU_MONITOR_BACKEND GPU_MONITOR_SAMPLE_TIMEOUT \
    GPU_MONITOR_PATH PERF_DEBUG_SUMMARY_ENABLE PERF_DEBUG_SUMMARY_PATH RAY_INCLUDE_DASHBOARD \
    MINIO3_RAY_STARTUP_RETRIES MINIO3_RAY_STARTUP_RETRY_DELAY_S \
    MINIO3_LB_ACQUIRE_RETRIES MINIO3_LB_ACQUIRE_RETRY_BASE_S MINIO3_LB_ACQUIRE_RETRY_MAX_S \
