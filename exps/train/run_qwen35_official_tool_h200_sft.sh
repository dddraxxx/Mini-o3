...
if [[ "${MINIO3_PRINT_CONFIG_ONLY:-0}" == "1" ]]; then
  for name in \
    ...
    WHOLE_CONVERSATION_TOKENIZE READ_PARQUET_DTYPE_BACKEND SP_SIZE FSDP_SIZE FSDP_STRATEGY FSDP_MODEL_DTYPE \
...
- Local adaptation: Qwen3.5-9B, Qwen3.5 official tool-call surface, add_vision_id=True, whole-conversation SFT tokenization, language-side LoRA rank $LORA_RANK on \`$LORA_TARGET_MODULES\`.
