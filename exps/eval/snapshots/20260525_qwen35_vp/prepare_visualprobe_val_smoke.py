...
PROJECT_ROOT = Path(os.environ.get("PROJECT_DIR", Path(__file__).resolve().parents[4]))
if str(PROJECT_ROOT) not in sys.path:
    sys.path.insert(0, str(PROJECT_ROOT))
if str(SNAPSHOT_DIR) not in sys.path:
    sys.path.insert(0, str(SNAPSHOT_DIR))

from preprocess_visualprobe import _convert_row
from preprocess_visualprobe import LEGACY_GROUNDING_PROMPT_SUITE
