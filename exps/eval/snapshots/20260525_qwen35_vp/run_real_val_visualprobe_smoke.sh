...
if [[ "${CHECK_QWEN35_ENV:-True}" == "True" && "$MODEL_PATH" == *"Qwen3.5"* ]]; then
  ...
  "${PYTHON_CMD_ARR[@]}" "$SNAPSHOT_DIR/check_qwen35_env.py" "${QWEN35_ENV_ARGS[@]}"
fi

"${PYTHON_CMD_ARR[@]}" "$SNAPSHOT_DIR/prepare_visualprobe_val_smoke.py" \
...
REWARD_FN_PATH=${REWARD_FN_PATH:-$SNAPSHOT_DIR/empty_reward.py} \
...
bash "$SNAPSHOT_DIR/run_qwen3_vl_8b_crop_lora_fsdp.sh" \
