...
PROJECT_DIR=${PROJECT_DIR:-$(cd "$SNAPSHOT_DIR/../../../.." && pwd)}
...
DEFAULT_TOOL_CONFIG_PATH="$SNAPSHOT_DIR/config/tool_config/minio3_crop_tool.yaml"
if [[ "$MINIO3_OFFICIAL_TOOL_NAME" == "image_zoom_in_tool" ]]; then
    DEFAULT_TOOL_CONFIG_PATH="$SNAPSHOT_DIR/config/tool_config/minio3_image_zoom_in_tool.yaml"
fi
TOOL_CONFIG_PATH=${TOOL_CONFIG_PATH:-$DEFAULT_TOOL_CONFIG_PATH}
REWARD_FN_PATH=${REWARD_FN_PATH:-$SNAPSHOT_DIR/minio3_reward.py}
...
if [[ -n "${RUN_DIR}" && ( "${GPU_MONITOR_ENABLE}" == "True" || "${GPU_MONITOR_ENABLE}" == "true" || "${GPU_MONITOR_ENABLE}" == "1" ) ]]; then
    ...
    "${PYTHON_CMD[@]}" "$SNAPSHOT_DIR/monitor_gpu_util.py" \
...
if [[ -n "${RUN_DIR}" && ( "${PERF_DEBUG_SUMMARY_ENABLE}" == "True" || "${PERF_DEBUG_SUMMARY_ENABLE}" == "true" || "${PERF_DEBUG_SUMMARY_ENABLE}" == "1" ) ]]; then
    ...
    "${PYTHON_CMD[@]}" "$SNAPSHOT_DIR/summarize_run_metrics.py" \
        "$RUN_DIR" \
