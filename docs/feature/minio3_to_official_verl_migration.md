...
新版有 70% 的替代能力：rollout JSONL、validation JSONL、metrics JSONL logger 都有；但 Mini-o3 专门的完整
  case/trajectory artifact 还没恢复。
  最小补齐方向不是复刻旧格式，而是：训练 wrapper 默认加 trainer.rollout_data_dir="$RUN_DIR/rollout_generations" 和
  logger 加 file；之后再扩展 AgentLoop 输出，把 tool calls / observation metadata 放进 dump。


旧 PyVision-style train 脚本默认会打开：

  +trainer.train_samples_jsonl="$RUN_DIR/train_samples.jsonl"
  +trainer.train_samples_jsonl_limit=16

  也就是 每个 training step 最多保存 16 条 train sample。如果当前 batch 少于 16，就保存 batch 里实际数量。

  旧版 train_samples.jsonl 里不是保存图片 bytes，而是保存 image path 引用：
  ...
  关键字段是 image_paths。所以它可以回看 real sample 对应的原图，但不是把图片嵌进 JSONL。

  另外旧版 eval 的 cases.jsonl 主要保存 text output / reward / token / timing，不明显保存 image_paths。真正带图路径
  的是 training 的 train_samples.jsonl。

  新版现在有 trainer.rollout_data_dir dump sample，但还没把 image_paths 默认补进去；这个可以很小地补齐成旧版等价功
  能。
